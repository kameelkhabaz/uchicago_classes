...
def has_an_infected_neighbor(city, location):
    ...
    left_idx = (location - 1) % len(city)
    right_idx = (location + 1) % len(city)

    # Check if neighboring people are infected
    left_disease_state, _ = city[left_idx]
    right_disease_state, _ = city[right_idx]

    return left_disease_state == "I" or right_disease_state == "I"
...
def advance_person_at_location(city, location, days_contagious):
    ...
    disease_state, days_in_state = city[location]
    
    if disease_state == "S":
        # If person is susceptible, check if neighbor is infected
        if has_an_infected_neighbor(city,location):
            return "I", 0
        else:
            return "S", days_in_state + 1
    elif disease_state == "I":
        # If person is infected, check if person remains infected
        if days_in_state + 1 < days_contagious:
            return "I", days_in_state + 1
        else:
            return "R", 0
    elif disease_state == "R" or disease_state == "V":
        # Recovered and vacinated people remain in the same state
        return disease_state, days_in_state + 1
...
def simulate_one_day(starting_city, days_contagious):
    ...
    for i , _ in enumerate(starting_city):
        person = advance_person_at_location(starting_city,i,days_contagious)
        city_one_day.append(person)
...
def is_transmission_possible(city):
    ...
    # Looking for susceptible people with infected neighbors
    for location, (disease_state, _) in enumerate(city):
...
def run_simulation(starting_city, days_contagious):
    ...
    # Create alias to prevent confusing variable names
    city = starting_city
...
def vaccinate_person(vax_tuple):
    ...
    disease_state, days_in_state, eagerness = vax_tuple

    # Only susceptible people can be vaccinated
    if disease_state == "S" and random.random() < eagerness:
        return "V", 0

    return disease_state, days_in_state
...
def vaccinate_city(city_vax_tuples, random_seed):
    ...
    for vax_tuple in city_vax_tuples:
        vaccinated_city.append(vaccinate_person(vax_tuple))
